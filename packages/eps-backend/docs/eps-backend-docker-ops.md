...
| console transaction history 502s, everything else fine | Only interaction `154` has its own upstream, so check `SIMPLIBANK_HISTORY_API_*` first. Read the `path` on the `eko_upstream` line — it is the URL actually posted to. Unset `_HOST`/`_PORT`/`_SCHEME` inherit the main upstream, but an unset `_PATH` defaults to `/ekoicici/v1/request` rather than following `SIMPLIBANK_API_PATH`, so a shared box on a different version silently misroutes. A non-JSON body (`error: "non-JSON response body"`, `nonJsonBytes` in place of a status summary) means whatever answered was not the interaction API — a proxy/WAF rejection and an outage look the same at `basic`; `EKO_LOG_LEVEL=full` quotes the body. |
