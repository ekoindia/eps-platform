...
4. Honor the documented branches: each carries either `onResponseTypeId` (match the response's `response_type_id`) or `onResponseStatusId` (match `response_status_id`) — jump to that branch's `goto` slug when it matches (e.g. `response_type_id` `308` → onboard the sender via `dmt-onboard-sender`), or stop when `goto` is `done`.
