...
final class EpsClientTest extends TestCase
{
    ...
    public function testAutoInjectsClientRefIdForNonGetCallWhenNotSupplied(): void
    {
        $client = new EpsClient('dev123', 'TEST_ACCESS_KEY_DO_NOT_USE', 'sandbox', now: fn () => 1700000000000);
        $target = $client->resolveTarget('pan-lite', [
            'initiator_id' => '9962981729',
        ...
        $this->assertNotEmpty($body['client_ref_id']);
    }

    public function testDoesNotOverrideExplicitClientRefIdOnNonGetCall(): void
    {
        $client = new EpsClient('dev123', 'TEST_ACCESS_KEY_DO_NOT_USE', 'sandbox', now: fn () => 1700000000000);
        $target = $client->resolveTarget('pan-lite', [
            'initiator_id' => '9962981729',
    ...
    public function testDoesNotInjectClientRefIdForGetCall(): void
    {
        $client = new EpsClient('dev123', 'TEST_ACCESS_KEY_DO_NOT_USE', 'sandbox', now: fn () => 1700000000000);
        $target = $client->resolveTarget('dmt-get-sender', [
            'customer_id' => '9123456789',
        ...
        // GET → body is null, client_ref_id must not appear in the URL
        $this->assertNull($target['body']);
        $this->assertStringNotContainsString('client_ref_id', $target['url']);
    }

    public function testSuccessiveNonGetCallsGetDistinctClientRefIds(): void
    {
        $client = new EpsClient('dev123', 'TEST_ACCESS_KEY_DO_NOT_USE', 'sandbox', now: fn () => 1700000000000);
        $params = [
            'initiator_id' => '9962981729',
        ...
        $target1 = $client->resolveTarget('pan-lite', $params);
        $target2 = $client->resolveTarget('pan-lite', $params);
        $body1 = json_decode($target1['body'], true);
        $body2 = json_decode($target2['body'], true);
        $this->assertNotSame($body1['client_ref_id'], $body2['client_ref_id']);
    }
}
