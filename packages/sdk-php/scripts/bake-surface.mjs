// Copies the site-generated SDK surface into the PHP package as a shipped asset.
// Run AFTER `npm run build` at the repo root (which emits
// dist/agent/sdk-surface.json). The PHP package has no npm scripts of its own,
// so this is invoked from the root `bake:all` script.
import { promises as fs } from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";

const here = path.dirname(fileURLToPath(import.meta.url));
const src = path.resolve(here, "../../../dist/agent/sdk-surface.json");
