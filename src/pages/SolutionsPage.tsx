	...
	return (
		<>
			<Helmet>
				...
				<meta
					name="description"
					content="Pre-bundled API packs for common business workflows — lending KYC, agent banking, merchant onboarding, fleet compliance, and more. Sandbox in minutes."
				/>
						...
						<div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
							...
							<FadeIn onView={false} delay={100} className="text-center">
								...
								<h1 className="text-4xl md:text-5xl font-bold text-white mb-4">
									Pre-bundled API packs
								</h1>
							</FadeIn>
							<FadeIn onView={false} delay={200} className="text-center">
								<p className="text-xl text-white/70 max-w-2xl mx-auto">
									Each pack bundles the exact APIs you need for a specific
									workflow — one integration, one contract, one dashboard.
								</p>
							</FadeIn>
						</div>
					</section>
					...
					{categories.map((cat, i) => (
						...
						>
							...
							<div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
								{ACTIVE_SOLUTIONS_LIST.filter((s) => s.category === cat).map(
									(sol, i) => (
										<SolutionCard
											key={sol.slug}
											solution={sol}
