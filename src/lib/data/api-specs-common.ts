...
export interface ApiParam {
	...
	description?: string;
	example?: unknown;
	/** Named syntactic format from {@link API_PARAM_FORMATS}; the SDKs match
	 * the wire string against its pattern. Build fails on an unknown name. */
	format?: string;
	/** Allowed values, compared as wire strings (`5` and `"5"` are the same). */
	enum?: (string | number)[];
	/** Inclusive numeric bounds; only meaningful on number/integer params. */
	min?: number;
	max?: number;
	/** Maximum length of the wire string in UTF-8 bytes. */
	maxLength?: number;
}
...
/**
 * Throws if any request param names a `format` missing from
 * {@link API_PARAM_FORMATS}, or if the registry itself is unusable. A dangling
 * format must fail the build, not a partner's request. Runs at build time via
 * `build-agent-bundle.ts`, alongside `assertRecipeSlugs`.
 */
export const assertParamFormats = (specs: ApiSpec[]): void => {
	assertFormatRegistry();
	for (const spec of specs) {
		for (const param of resolveRequestParams(spec)) {
			if (param.format !== undefined && !(param.format in API_PARAM_FORMATS)) {
				throw new Error(
					`api-specs: spec "${spec.id}" param "${param.name}" names unknown format "${param.format}".`,
				);
			}
		}
	}
