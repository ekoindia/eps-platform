...
<Callout type="note">
Already have a scanner? Test it right here — no code, no signup. The tester at
the bottom of this page checks your device and driver in a few seconds.

<Button
	asChild
	variant="outline"
	size="sm"
	className="not-prose mt-3 border-sky-600/40 bg-white/70 font-semibold text-sky-800 no-underline hover:bg-sky-100 hover:text-sky-900 dark:border-sky-400/40 dark:bg-transparent dark:text-sky-200 dark:hover:bg-sky-900/40 dark:hover:text-sky-100"
>
	<a href="#test-your-device-setup">Test my biometric device ↓</a>
</Button>
...
| `otp`     | Both        | string                           | Only for OTP-combined flows — e.g. the customer's 6-digit SMS OTP for an [AePS cash withdrawal above ₹5,000](/docs/aeps-fingpay-cash-withdrawal). Otherwise empty. |
