...
| ----------------- | ----------------------------------------------------- | ------------------------------------------------------------------------ |
| **Access Key**    | Shared once, kept server-side only                    | Never sent in a request; used to compute `secret-key` and `request_hash` |
...
| `request_hash`         | money-debit only | HMAC over the API's signed parameters (see below).     |
...
## Computing the `request_hash` (financial APIs)

Money-debit endpoints require an extra signature over the request parameters:

1. Concatenate the API-specific parameters **in the documented order**.
2. Base64-encode the `access_key`.
3. Compute `HMAC-SHA256(concatenated-params, base64(access_key))`.
4. Base64-encode the result — that's the `request_hash`.

The parameter sequence varies per API; each financial endpoint's page lists the
order it expects.
...
window. When using the in-page **Test Request** console, signing happens locally
in your browser with the UAT credentials you supply; they are never sent
anywhere except the sandbox API itself.
