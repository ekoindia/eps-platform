...
| PII (PAN/Aadhaar/bank data) | Bodies and headers are never logged — access log carries tool *name* only, extracted from a guarded clone of the request. Error messages are curated (`sanitizeError`): upstream/network error text is replaced (it can echo request data); only EpsClient validation messages (param *names*) pass through. |
...
3. **Manual deploy for now**: the poller reconciles only the eps-backend image. Because the poller is env-driven (`IMAGE`/`WATCH_TAG`), auto-deploy is a compose-only follow-up (second poller service instance watching this image).
...
- Shared-KV rate limiting, npm publication of the stdio bin.
