...
| AePS Cash Withdrawal OTP          | `aeps-fingpay-cash-withdrawal-otp` | POST   | `/customer/collection/aeps-fingpay/cash-withdrawal/otp/{customer_id}` | new (2026-09) — required before withdrawals > ₹5,000; this path spells `withdrawal` correctly |
...
**Added (2026-09-11, Fingpay OTP-gated flow doc):** withdrawals above ₹5,000 first call
`aeps-fingpay-cash-withdrawal-otp` → `fp_transaction_id` (1459), sent to Cash Withdrawal as
`txn_otp_request_id`, with the customer's SMS OTP in the PidOptions `otp` attribute. Skipping the
OTP call makes Cash Withdrawal return 1459 with nothing withdrawn. Recipe models the step with
`appliesWhen`. **Pending backend confirmation:** `bank_code` optionality on the OTP call, the 1459
fallback, `env` value in PidOptions (source doc says `PROD`, RDService guide says `P`).
...
| `operator_id` on Fetch Bill / Pay Bill                                        | **`phone_operator_code`** (still `operator_id` in the Get Operators _response_) |
...
- **Groups render in first-appearance file order**, so the spec array order in `api-specs.ts` is
  load-bearing — the ungrouped fetch/pay pair must stay _after_ both grouped blocks to hoist last.
